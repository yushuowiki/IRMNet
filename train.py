...
# loss_window = viz.line( Y=torch.zeros((1)).cpu(), X=torch.zeros((1)).cpu(), opts=dict(xlabel='epoch', ylabel='Loss', title='loss'))
# grad_window = viz.line(Y=torch.zeros((1)).cpu(), X=torch.zeros((1)).cpu(),
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = IRMNet(layers=args.layers, classes=2, zoom_factor=8, \
    ...
    optimizer = torch.optim.SGD(
        ...
        # {'params': model.cls2.parameters()},
        {'params': model.s_down.parameters()},
...
def train(train_loader, model, optimizer, epoch):
    ...
    # schedule_sampler = create_named_schedule_sampler(args.schedule_sampler, diffusion,  maxt=100)
    model.train()
    end = time.time()
...
def validate(val_loader, model, criterion):
    ...
    iter_num = 0
    total_time = 0
    pred_list=torch.load("/home/dell2/YY/FSS-DDPM/pred3.pth")
    for e in range(10):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            pred=pred_list[i]
            # viz.image(visualize(pred.cpu()[0, -1,...]), opts=dict(caption="sample"+ str(i) ))
            # if(i==25):
            #     break
            output = model(s_x=s_input, s_y=s_mask, x=input, y=target,pred=pred) # pred diffusion
