...
from guided_diffusion.RRDB import RRDB
...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    ...
    if schedule_name == "linear":
        ...
        # diffusion steps.
        scale = 100 / num_diffusion_timesteps#aaa
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.f = nn.Sequential(
            nn.Conv2d(1, 256, kernel_size=1, padding=0, bias=False),
            nn.ReLU(inplace=True),
            nn.Dropout2d(p=0.5)                   
        ) 
        BatchNorm = nn.BatchNorm2d
        self.rrdb=RRDB(layers=50, BatchNorm=BatchNorm, pretrained=True, vgg=False)
    ...
    def p_mean_variance(
        self, model, x,fea, t, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        xt=self.f(x)
        x_s=xt+fea
        model_output = model(x_s, self._scale_timesteps(t), **model_kwargs)
    ...
    def _scale_timesteps(self, t):
        if self.rescale_timesteps:
            ...
            return t.float() * (100.0 / self.num_timesteps)#aaa
        return t
    ...
    def p_sample_loop_progressive(
        ...
        # img=F.interpolate(img, size=(60, 60), mode='bilinear', align_corners=True)
    ...
    def _vb_terms_bpd(
        self, model, x_start, x_t,fea, t, clip_denoised=True, model_kwargs=None
    ):
        """
        ...
        out = self.p_mean_variance(
            model, x_t,fea, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )
        kl = normal_kl(
    ...
    def training_losses_segmentation(self, model,  x_start, t, model_kwargs=None, noise=None):
        """
        ...
        x_start = F.interpolate(x_start, size=(60, 60), mode='bilinear', align_corners=True)
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            if self.model_var_type in [
                ...
                if self.loss_type == LossType.RESCALED_MSE:
                    ...
                    terms["vb"] *= self.num_timesteps / 25.0
